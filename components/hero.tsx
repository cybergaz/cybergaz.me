...
const Hero = () => {
    ...
    const debug = "bg-emerald-500/5"

    return (
        <section id='home' className={`${debug} pt-44 max-w-[100rem] px-5 mx-auto`}>
            <div className="grid grid-cols-2 grid-rows-2 csm:grid-cols-1 csm:grid-rows-4">
                ...
                >
                    <div className="flex justify-start items-center space-x-2 text-2xl font-nunito font-bold text-zinc-400 pl-0.5" >
                        <span> Hi </span>
                ...
                <MotionDiv className={`${debug} relative flex justify-end items-center place-self-end self-center csm:place-self-center csm:order-1`} >
                    <Image className="dark:invert " src="/images/laptop.svg" alt="laptop svg" width={510} height={500} />
                    <ImagesSlider className="absolute inset-0 top-[0.3rem] right-[0.1rem] mx-auto w-[80%] h-[91.8%]" images={images} width={800} height={800} overlay={false}> loading... </ImagesSlider>
                    <div className="absolute font-monte text-lg csm:text-base opacity-0 hover:opacity-100 flex justify-center items-center inset-0 top-[0.3rem] right-[0.1rem] mx-auto w-[80%] h-[91.8%] backdrop-blur-sm transition-all duration-500" > my development environment</div>
                </MotionDiv>

                <MotionDiv
                    className={`${debug} space-x-7 flex flex-wrap justify-start csm:justify-center items-center gap-4 csm:order-3`}
                >
